...
const Diseases = () => {
  ...
  const fetchInstituteName = async (id) => {
    try {
      const res = await axios.get(
        `${BACKEND_URL}:${BACKEND_PORT_NO}/institute-api/institution/${id}`
      );
  ...
  const fetchEmployees = async () => {
    try {
      const res = await axios.get(
        `${BACKEND_URL}:${BACKEND_PORT_NO}/employee-api/employees`
      );
      ...
      try {
        const altRes = await axios.get(
          `${BACKEND_URL}:${BACKEND_PORT_NO}/employee-api/all`
        );
  ...
  useEffect(() => {
    const fetchFamilyMembers = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `${BACKEND_URL}:${BACKEND_PORT_NO}/family-api/family/${formData.Employee_ID}`
        );
        ...
        if (!res.data || res.data.length === 0) {
          ...
          const employeeRes = await axios.get(
            `${BACKEND_URL}:${BACKEND_PORT_NO}/employee-api/profile/${formData.Employee_ID}`
          );
            ...
            const familyPromises = employeeRes.data.FamilyMembers.map(async (familyId) => {
              try {
                const familyRes = await axios.get(
                  `${BACKEND_URL}:${BACKEND_PORT_NO}/family-api/member/${familyId}`
                );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${BACKEND_URL}:${BACKEND_PORT_NO}/disease-api/diseases`,
        payload
      );
