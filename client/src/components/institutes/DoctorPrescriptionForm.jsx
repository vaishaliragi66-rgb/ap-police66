 ...
 const loadEmployeeReports = async () => {
  ...
  try {
    const res = await axios.get(
      `${import.meta.env.REACT_APP_API_URL}/employee-api/health-report-detailed`,
      {
        params: {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.REACT_APP_API_URL}/diagnosis-api/tests`)
...
useEffect(() => {
  axios
    .get(`${import.meta.env.REACT_APP_API_URL}/xray-api/types`)
  ...
  useEffect(() => {
    ...
    axios
      .get(`${import.meta.env.REACT_APP_API_URL}/institute-api/inventory/${formData.Institute_ID}`)
      .then(res => {
  ...
  const fetchDiseases = async (employeeId) => {
    try {
      const res = await axios.get(
        `${import.meta.env.REACT_APP_API_URL}/disease-api/employee/${employeeId}`
      );
...
if (selectedMedicines.length === 0) {
  ...
  return;
    ...
    await axios.post(`${import.meta.env.REACT_APP_API_URL}/api/medical-actions`, {
...
if (validXrays.length === 0) {
  ...
  await axios.post(
    `${import.meta.env.REACT_APP_API_URL}/api/medical-actions`,
    {
      employee_id: formData.Employee_ID,
      visit_id: formData.visit_id || null,
