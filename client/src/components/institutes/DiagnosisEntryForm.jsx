...
const fetchDoctorDiagnosis = async (visitId) => {
  try {
    const res = await axios.get(
      `${BACKEND_URL}:${BACKEND_PORT_NO}/diagnosis-api/visit/${visitId}/doctor`
    );
...
useEffect(() => {
  ...
  const fetchInstituteName = async (id) => {
    try {
      const res = await axios.get(`${BACKEND_URL}:${BACKEND_PORT_NO}/institute-api/institution/${id}`);
...
const fetchTests = async () => {
  try {
    const res = await axios.get(
      `${BACKEND_URL}:${BACKEND_PORT_NO}/diagnosis-api/tests`
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${BACKEND_URL}:${BACKEND_PORT_NO}/diagnosis-api/add`,
        {
          Institute_ID: formData.Institute_ID,
          Employee_ID: formData.Employee_ID,
          IsFamilyMember: formData.IsFamilyMember,
...
const fetchVisitDetails = async (visitId) => {
  const res = await axios.get(
    `${BACKEND_URL}:${BACKEND_PORT_NO}/visit-api/visit/${visitId}`
  );
  return res.data;
};
const fetchPastRecords = async () => {
  ...
  try {
    const res = await axios.get(
      `${BACKEND_URL}:${BACKEND_PORT_NO}/diagnosis-api/records/${formData.Employee_ID}?isFamily=${formData.IsFamilyMember}&familyId=${formData.FamilyMember_ID}`
    );
