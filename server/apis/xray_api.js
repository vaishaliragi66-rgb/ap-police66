...
// // ==========================================
// // GET master X-ray list
// // ==========================================
// xrayApp.get("/xrays", async (req, res) => {
//   try {
//     const xrays = await Xray.find().sort({ Body_Part: 1 });
//     res.status(200).json(xrays);
//   } catch (err) {
//     console.error("Error fetching xrays:", err);
//     res.status(500).json({ error: "Failed to fetch xrays" });
//   }
// });


// // ==========================================
// // Add master X-ray type
// // ==========================================
// xrayApp.post("/xrays/add", async (req, res) => {
//   try {
//     const { Body_Part, Side, View, Film_Size } = req.body;

//     if (!Body_Part) {
//       return res.status(400).json({ message: "Body_Part required" });
//     }
...
//     res.status(201).json({ message: "X-ray created", xray: newXray });
//   } catch (err) {
//     console.error("Error adding xray:", err);
//     res.status(500).json({ error: "Failed to add xray" });
//   }
// });


// // ==========================================
// // Add or update X-ray record
// // ==========================================
// // ✅ Add an X-ray record
// // xrayApp.post("/add", async (req, res) => {
// //   try {
// //     const {
// //       Institute_ID,
// //       Employee_ID,
...
// //     if (!Institute_ID || !Employee_ID || !Array.isArray(Xrays) || Xrays.length === 0) {
...
// //         Xrays: Xrays.map(x => ({
// //           Xray_ID: x.Xray_ID || null,
// //           Xray_Type: x.Xray_Type || "",   // 👈 NEW FIELD
...
// //           existingXray.Side = x.Side || existingXray.Side;
// //           existingXray.Film_Size = x.Film_Size || existingXray.Film_Size;
// //           existingXray.Findings = x.Findings || existingXray.Findings;
// //           existingXray.Impression = x.Impression || existingXray.Impression;
// //           existingXray.Remarks = x.Remarks || Xray_Notes || existingXray.Remarks;
...
// //       });
// //     } catch (logErr) {
// //       console.error("⚠️ MedicalAction log failed (xray):", logErr.message);
// //     }
...
// //       Notes: Xray_Notes || `Added ${Xrays.length} X-ray(s)`
// //     };
...
// //     return res.status(200).json({ message: "X-ray record saved or updated" });
...
//     if (!Institute_ID || !Employee_ID || !Array.isArray(Xrays)) {
//       return res.status(400).json({ message: "Missing required fields" });
//     }

//     // Remove invalid rows
//     const cleanedXrays = Xrays.filter(
//       x => x.Xray_Type && x.Body_Part
//     );

//     if (cleanedXrays.length === 0) {
//       return res.status(400).json({
//         message: "No valid X-ray entries"
//       });
//     }
...
//     res.status(200).json({
//       message: "X-ray record saved successfully"
//     });
...
//     console.log("Incoming Xrays:", Xrays);
//     console.log("Existing Record Xrays:", record?.Xrays);
