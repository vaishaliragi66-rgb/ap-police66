...
(async ()=>{
  ...
  console.log('diagnosistests total:', total);
  const sample = await col.find({}).limit(10).toArray();
  sample.forEach(s=> console.log({ Test_Name: s.Test_Name, Group: s.Group, Reference_Range: s.Reference_Range, Units: s.Units }));
