...
(async ()=>{
  ...
  if (!instituteId) { console.error('Usage: node insert_custom_panel.js <instituteId>'); process.exit(1); }

  const backupDir = `server/exports/insert_custom_panel_${instituteId}_${new Date().toISOString().replace(/[:.]/g,'-')}`;
  fs.mkdirSync(backupDir, { recursive: true });

  const report = { instituteId, categories: [] };

  for (const cat of panel) {
    const normCat = normalize(cat.category);
    const catDoc = await MasterCategory.findOneAndUpdate(
      { Institute_ID: instituteId, normalized_name: normCat },
      { $set: { category_name: cat.category, normalized_name: normCat, status: 'Active' } },
      { upsert: true, new: true }
    );

    const catReport = { category: cat.category, upserts: [] };
    for (const t of cat.tests) {
      const normVal = normalize(t.name);
      try {
        const mv = await MasterValue.findOneAndUpdate(
          { Institute_ID: instituteId, category_id: catDoc._id, normalized_value: normVal },
          { $set: { value_name: t.name, normalized_value: normVal, status: 'Active', meta: { kind: 'test', reference: t.reference, unit: t.unit, source: 'custom_panel_2026-04-15' } } },
          { upsert: true, new: true }
        );
        catReport.upserts.push({ name: t.name, id: mv._id.toString() });
      } catch (e) {
        catReport.upserts.push({ name: t.name, error: e.message });
      }
    }
    report.categories.push(catReport);
  }

  fs.writeFileSync(`${backupDir}/insert_report.json`, JSON.stringify(report, null, 2));
  console.log('Insert complete. Report:', `${backupDir}/insert_report.json`);
