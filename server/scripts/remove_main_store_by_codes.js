...
async function run() {
  ...
  const deletedMedicines = await MainStoreMedicine.deleteMany({
    Institute_ID: institute._id,
    _id: { $in: ids }
  });

  console.log(`Deleted ledger rows: ${deletedLedger.deletedCount}`);
  console.log(`Deleted main-store medicines: ${deletedMedicines.deletedCount}`);
}

if (require.main === module) {
  run()
    .catch((err) => {
      console.error("Failed to remove main-store medicines:", err);
      process.exitCode = 1;
    })
    .finally(async () => {
      try {
